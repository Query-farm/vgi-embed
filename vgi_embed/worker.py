...
DuckDB spawns this over stdio and ATTACHes it::
    ...
    ATTACH 'embed' (TYPE vgi, LOCATION 'vgi-embed-worker');
...
This module is wheel-importable (the console script ``vgi-embed-worker`` and the
Docker HTTP entrypoint both target it); the repo-root ``embed_worker.py`` is a
thin PEP 723 shim that re-exports :class:`EmbedWorker` and :func:`main` so
``uv run embed_worker.py`` keeps working unchanged.
"""
...
def _models_values_sql() -> str:
    ...
    Returns:
        A ``SELECT ... FROM (VALUES ...)`` SQL string projecting ``model`` and ``dim``.
    """
                    ...
                    [
                        {
                            ...
                            "title": "Embedding",
                            "description": (
                                "Turn text into fixed-length FLOAT[] vectors for symmetric text-to-text comparison."
                            ),
                        },
                        {
                            "name": "retrieval",
