...
_EXECUTABLE_EXAMPLES = """[
  ...
  {
    "description": "Embed a corpus passage (no query prefix) into a 384-dim FLOAT[].",
    "sql": "SELECT len(embed.main.embed_passage('Reset your password from the account settings page.')) AS dim"
  },
  {
    "description": "Look up a model's output dimension without loading the model.",
    "sql": "SELECT embed.main.embedding_dim('BAAI/bge-base-en-v1.5') AS dim"
  },
  {
    "description": "Report the worker + fastembed backend + default-model identity string.",
    "sql": "SELECT embed.main.embed_version() AS version"
  }
]"""
