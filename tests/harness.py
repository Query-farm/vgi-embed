"""In-process VGI invocation helpers for the embed worker test suite.

Scalars are driven through ``compute`` directly (its declarative array-in /
array-out contract); the discovery table function goes through the real
bind -> init -> process lifecycle (adapted from the vgi-conform suite) so its
fixed output schema is exercised. A small skip guard reports whether the default
fastembed model is available locally, so model-download tests skip cleanly
offline (cold cache, no network) but run wherever the model is present.
"""
...
def model_available() -> bool:
    """Whether the default fastembed model can be loaded (for skip guards).

    True once the ONNX model is cached locally (or downloadable). Lets the
    embedding tests self-skip on a bare, offline checkout while still running
    wherever the model has been provisioned.
    """
...
def invoke_table_function(func_cls: type) -> pa.Table:
